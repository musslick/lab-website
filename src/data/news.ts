...
export const newsItems: NewsItem[] = [
  {
    ...
    tags: ["Publication", "Neural Networks"],
    featured: true
  },
  {
    id: "news-2",
...
// Helper function to check if news items exist in localStorage
export const ensureNewsItemsInLocalStorage = () => {
  const savedNews = localStorage.getItem('newsItems');
  
  if (!savedNews) {
    // If no news items in localStorage, initialize with default data
    console.log("Initializing news items in localStorage");
    localStorage.setItem('newsItems', JSON.stringify(newsItems));
  }
};

// Call the function to ensure we have news items
ensureNewsItemsInLocalStorage();
